...
│  Position sizing · Drawdown guard + flatten · Intent-based executor  │
└─────────────────────────────────────────────────────────────────────┘
```
...
- Model types: HistGradientBoosting (champion), LogReg, Random Forest, XGBoost, LightGBM
...
- Max drawdown guard: halts trading **and flattens all positions** on breach
- Portfolio simulation with fees (`PaperPortfolio`)

### Execution
- **Intent-based executor API**: `open_long`, `close_long`, `open_short`, `close_short`, `emergency_flatten`
...
| `--model-type` | `logreg` | `logreg` / `random_forest` / `xgboost` / `lightgbm` / `hist_gb` |
